...
public class AppMain {
    ...
    private static  int s_mid = 0;//设备id
    private static int s_uid = 0;//用户Id
    private static  int s_goodsid = 0;//商品id

    public static void main(String[] args) {
        // 参数一：控制发送每条的延时时间，默认是0
        long delay = args.length > 0 ? Long.parseLong(args[0]) : 0L;
        // 参数二：循环遍历次数
        long loop_len = args.length>1?Long.parseLong(args[1]):1000;
        //生成数据
        generateLog(delay,loop_len);
    }

    private static void generateLog(long delay, long loop_len) {
